...
const _kWeekdayOptions = [
  ...
  (5, "Fri"),
  (6, "Sat"),
]; // Sunday closed, same as everywhere else
...
class _AdvancedBookingScreenState extends ConsumerState<AdvancedBookingScreen> {
  ...
  List<Trainer> _matchCoaches(List<Trainer> trainers, String? city) {
    bool offersIt(Trainer t) {
      for (var wd = 0; wd <= 6; wd++) {
        if (trainerOfferings(t, wd).any(
          (o) => o.sessionType == _sessionType && o.discipline == _discipline,
        ))
          return true;
      }
      return false;
    ...
    final inCity = offering.where((t) {
      ...
      return trainerCity != null &&
          trainerCity.trim().toLowerCase() == city.trim().toLowerCase();
    }).toList();
  ...
  void _computeOccurrences({
    ...
    final totalWeeklySlots = _brackets.fold<int>(
      0,
      (sum, b) => sum + b.days.length,
    );
    final weeksCount = _rangeChoice == "2w"
        ? 2
        : _rangeChoice == "4w"
        ? 4
        : (remaining / (totalWeeklySlots == 0 ? 1 : totalWeeklySlots))
              .ceil()
              .clamp(1, 1 << 30);
    ...
    for (var w = 0; w < weeksCount; w++) {
      for (final bracket in _brackets) {
        for (final wd in bracket.days) {
          ...
          if (dateCmp > 0 || (dateCmp == 0 && bracket.time > nowMin))
            candidates.add((date, wd, bracket.time));
        }
      }
    }
    candidates.sort((a, b) {
    ...
    _occurrences = candidates.map((c) {
      ...
      if (match == null) {
        if (!anyOffered) {
          return _Occurrence(
            ...
            slot: slot,
            status: "skipped",
            reason: "No coach offers this at that time",
          );
        }
      ...
      return _Occurrence(
        ...
        status: "skipped",
        reason: !budgetLeft
            ? "No sessions left in your plan"
            : (check.msg ?? check.reason ?? "Unavailable"),
      );
    }).toList();
  }
  ...
  Future<void> _confirm({
    ...
    for (final o in _occurrences) {
      if (o.status == "bookable" && o.trainerId != null) {
        ...
        Trainer? anyTeaching;
        for (final t in _eligibleTrainers) {
          if (trainerOfferings(t, o.weekday).any(
            (of) =>
                of.sessionType == _sessionType &&
                of.discipline == _discipline &&
                of.slot == o.slot,
          )) {
            anyTeaching = t;
            break;
        ...
        try {
          final entry = await SupabaseService.insertWaitlistEntry(
            WaitlistEntry(
              id: "",
              clientId: info.id,
  ...
  Widget build(BuildContext context) {
    ...
    switch (_step) {
      ...
      case "discipline":
        ...
        final options = kDisciplineLabels.entries
            .where((e) => offered.contains(e.key))
            .toList();
        void backToType() => setState(() => _step = "type");
        return LocalBackScope(
          isOpen: true,
          onBack: backToType,
          child: _StepScaffold(
            breadcrumb: _breadcrumb(backToType),
            title: "Discipline",
            children: options.isEmpty
                ? [
                    HintBox(
                      text:
                          "No ${sessionTypeLabel(_sessionType!).toLowerCase()} sessions are set up yet.",
                    ),
                  ]
                : options
                      .map(
                        (e) => _OptionCard(
                          ...
                          onTap: () => setState(() {
                            _discipline = e.key;
                            _eligibleTrainers = _matchCoaches(
                              trainers,
                              info.city,
                            );
                            _brackets
      ...
      case "pattern":
        void backToDiscipline() => setState(() => _step = "discipline");
        if (_eligibleTrainers.isEmpty) {
          return LocalBackScope(
            isOpen: true,
            onBack: backToDiscipline,
            child: Padding(
              padding: const EdgeInsets.all(18),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.stretch,
                children: [
                  _breadcrumb(backToDiscipline),
                  const SizedBox(height: 10),
                  HintBox(
                    text:
                        "No coach currently offers ${disciplineLabel(_discipline!)}.",
                  ),
                ],
              ),
            ),
            ...
            child: Column(
              ...
              children: [
                ...
                ..._brackets.asMap().entries.map(
                  (entry) => _BracketEditor(
                    index: entry.key,
                    bracket: entry.value,
                    canRemove: _brackets.length > 1,
                    onChanged: () => setState(() {}),
                    onRemove: () =>
                        setState(() => _brackets.removeAt(entry.key)),
                  ),
                ),
                Padding(
                ...
                BtnGold(
                  onPressed: _brackets.any((b) => b.days.isNotEmpty)
                      ? () => setState(() => _step = "range")
                      : null,
                  full: true,
      ...
      case "range":
        ...
        void backToPattern() => setState(() => _step = "pattern");
        return LocalBackScope(
          isOpen: true,
          onBack: backToPattern,
          child: _StepScaffold(
            ...
            children: [
              ...
              _OptionCard(
                ...
                sub:
                    "Books out as many of your $remaining remaining session${remaining != 1 ? "s" : ""} as your pattern allows.",
      ...
      case "review":
        final bookableCount = _occurrences
            .where((o) => o.status == "bookable")
            .length;
        ...
        return SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 10),
              SectionLabel(
                "Review — ${_occurrences.length} session${_occurrences.length != 1 ? "s" : ""} in range",
              ),
              Text(
              ...
              if (fullOnes.isNotEmpty)
                const Padding(
                  padding: EdgeInsets.only(top: 4, bottom: 4),
                  child: HintBox(
                    text:
                        "Waitlist requests need the owner's approval before they're confirmed bookings.",
                  ),
                ),
              const SizedBox(height: 14),
              BtnGold(
                full: true,
                onPressed: (_busy || _occurrences.isEmpty || unresolvedFull)
                    ? null
                    : () => _confirm(info: info, trainers: trainers),
                child: Text(
                  _busy
                      ? "Booking…"
                      : unresolvedFull
                      ? "Resolve full sessions above"
                      : "Confirm $bookableCount booking${bookableCount != 1 ? "s" : ""}",
                ),
              ),
            ],
          ),
                ...
                child: Column(
                  ...
                  children: [
                    Padding(
                      ...
                      child: Text(
                        "✓ ${s.booked} session${s.booked != 1 ? "s" : ""} booked",
                    ...
                    Padding(
                      padding: const EdgeInsets.symmetric(vertical: 4),
                      child: Text(
                        "⏳ ${s.waitlisted} waitlist request${s.waitlisted != 1 ? "s" : ""} submitted — pending owner approval",
                        style: const TextStyle(fontSize: 13),
                      ),
                    ),
                    if (s.skipped > 0)
      ...
      child: Container(
        ...
        child: Text(
          ...
          style: TextStyle(
            fontSize: 11,
            fontWeight: FontWeight.w700,
            color: selected ? color : AppColors.mute,
          ),
        ),
      ),
    );
                ...
                onTap: () async {
                  final picked = await showTimePicker(
                    ...
                    initialTime: TimeOfDay(
                      hour: bracket.time ~/ 60,
                      minute: bracket.time % 60,
                    ),
                  );
                  if (picked != null) {
