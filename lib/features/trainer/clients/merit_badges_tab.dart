...
final _coachAwardable = kMeritBadges
    .where((b) => b.category == "coach" && b.key != "gym_citizen")
    .toList();
...
class _MeritBadgesTabState extends ConsumerState<MeritBadgesTab> {
  ...
  Future<void> _refreshBadgesAndPoints() async {
    ...
    final freshBadges = await SupabaseService.loadMeritBadgesFor(
      widget.clientId,
    );
    ref
        .read(earnedBadgesProvider.notifier)
        .replaceForClient(widget.clientId, freshBadges);
    final freshPoints = await SupabaseService.loadPointsLedgerFor(
      widget.clientId,
    );
    ref
        .read(pointsLedgerProvider.notifier)
        .replaceForClient(widget.clientId, freshPoints);
  }
  ...
  Widget build(BuildContext context) {
    ...
    final activeBadges =
        myBadges
            .where(
              (b) => b.isActive && kMeritBadges.any((m) => m.key == b.badgeKey),
            )
            .toList()
    ...
    final historyBadges =
        myBadges
            .where(
              (b) =>
                  !b.isActive && kMeritBadges.any((m) => m.key == b.badgeKey),
            )
            .toList()
          ..sort((a, b) => (b.revokedAt ?? "").compareTo(a.revokedAt ?? ""));
    final activeKeys = activeBadges.map((b) => b.badgeKey).toSet();
    final awardable = _coachAwardable
        .where((b) => !activeKeys.contains(b.key))
        .toList();
    final forceAssignable = kMeritBadges
        .where((b) => !activeKeys.contains(b.key))
        .toList();
    final gymCitizenActiveSubCount = myBadges
        .where(
          (b) => b.isActive && kGymCitizenSubBadgeKeys.contains(b.badgeKey),
        )
        .length;
    ...
    void award(MeritBadgeDef badge) async {
      ...
      try {
        await SupabaseService.grantMeritBadge(
          widget.clientId,
          badge.key,
          cleanNote,
        );
        ...
        if (badge.key == "record_breaker" && context.mounted) {
          final trainerId = trainerAuth != "owner"
              ? trainerAuth
              : info.primaryTrainerId;
          if (trainerId != null) {
            final exerciseNames =
                record?.programDays
                    .expand((d) => d.exercises.map((e) => e.name))
                    .toSet()
                    .toList() ??
                <String>[];
            final exercise = await _promptExercise(context, exerciseNames);
            if (exercise != null && exercise.isNotEmpty) {
              await SupabaseService.insertCoachPrEvent(
                clientId: widget.clientId,
                trainerId: trainerId,
    ...
    void revoke(EarnedBadge badge, String name) async {
      final confirmed = await _confirm(
        context,
        'Remove "$name" from ${info.name}? It moves to History and can be awarded again later.',
      );
      if (!confirmed) return;
      try {
    ...
    void forceAward() async {
      ...
      final confirmed = await _confirm(
        context,
        'Force-assign "${meta?.name ?? pick}" to ${info.name}? This bypasses normal eligibility checks.',
      );
      if (!confirmed || !context.mounted) return;
      final note = await _promptText(
        context,
      ...
      try {
        await SupabaseService.forceAwardMeritBadge(
          widget.clientId,
          pick,
        ...
        children: [
          ...
          InkWell(
            ...
            child: Container(
              ...
              child: Row(
                children: [
                  if (gymCitizenActiveSubCount > 0 &&
                      gymCitizenActiveSubCount < 10)
                    GymCitizenProgressRing(
                      activeCount: gymCitizenActiveSubCount,
                      size: 30,
                      child: const MeritBadgeShield(
                        badgeKey: "gym_citizen",
                        size: 20,
                        grayscale: true,
                      ),
                    )
                    ...
                    MeritBadgeShield(
                      badgeKey: "gym_citizen",
                      size: 30,
                      grayscale: gymCitizenActiveSubCount < 10,
                    ),
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text(
                          "Gym Citizen — $gymCitizenActiveSubCount/10 verified",
                        ...
                        const Text(
                          "Check off the 10 sub-badges — the badge itself awards automatically at 10/10.",
                ...
                children: [
                  const Row(
                    children: [
                      ...
                      SizedBox(width: 6),
                      Text(
                        "OWNER OVERRIDE — ASSIGN ANY BADGE",
                        style: TextStyle(
                          fontSize: 11,
                          color: AppColors.gold,
                          fontWeight: FontWeight.w700,
                    ...
                    children: [
                      Expanded(
                        child: DropdownButtonFormField<String>(
                          ...
                          items: forceAssignable
                              .map(
                                (b) => DropdownMenuItem(
                                  value: b.key,
                                  child: Text(
                                    "${b.name} (${b.category == 'coach' ? 'Coach Awarded' : 'Automatic'})",
                                  ),
                                ),
                              )
                              .toList(),
                      ...
                      BtnGold(
                        onPressed: _forcePick == null ? null : forceAward,
                  ...
                  child: Column(
                    children: [
                      ...
                      if (b.earnedMethod == "coach" &&
                          b.grantedByUserId != null)
                        Text(
                          "by ${staffNames[b.grantedByUserId] ?? 'a coach'}",
                          style: const TextStyle(
                            fontSize: 11,
                            color: AppColors.mute,
                          ),
          ...
          children: [
            if (suggestions.isNotEmpty) ...[
              DropdownButtonFormField<String>(
                ...
                hint: const Text(
                  "Pick from this client's program…",
...
class _GymCitizenChecklist extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_GymCitizenChecklist> createState() =>
      _GymCitizenChecklistState();
}
...
class _GymCitizenChecklistState extends ConsumerState<_GymCitizenChecklist> {
  ...
  String _fmtExpiry(String iso) {
    final d = DateTime.tryParse(iso);
    if (d == null) return "—";
    return _fmtBadgeDate(
      DateTime(d.year, d.month + 6, d.day).toIso8601String(),
    );
  }
  ...
  Widget build(BuildContext context) {
    final all = ref.watch(earnedBadgesProvider);
    final myBadges = all
        .where((b) => b.clientId == widget.clientId && b.isActive)
        .toList();
    final activeCount = kGymCitizenSubBadges
        .where((sub) => myBadges.any((b) => b.badgeKey == sub.key))
        .length;
